...
const CommonModal = ({content, submitBtnText, cancelBtnText, onConfirm}) => {
    ...
    const clickModal = (confirm) => {
        console.debug("clickModal: ", confirm)
        onConfirm(confirm)
        setIsOpen(false)
    }
        ...
        <React.Fragment>
            {isOpen &&
                <div tabIndex="-1"
                     ...
                     onClick={(event) => clickModal(false)}
...
const TodoListCount = ({items}) => {
    ...
    return (
        <h3 id="todo-count" className="text-1xl text-center pb-5">
            모든 할 일 {items.length}개 / 남은 할 일 {completeCount}개
        </h3>
    )
}
...
const TodoItem = ({item, onUpdate}) => {
    ...
    return (
        <React.Fragment>
            ...
            <li className="pt-3 pb-3 sm:pt-4">
                <div className="flex items-center space-x-4 rtl:space-x-reverse">
                    <div className="flex-shrink-0" onClick={updateItem}>
                        <i className={`fa-solid fa-check text-2xl cursor-pointer ${item.completed ? 'text-green-500' : 'text-neutral-300'}`}></i>
                    </div>
                    ...
                    <div
                        ...
                        <i className="fa-solid fa-circle-xmark cursor-pointer text-neutral-300"
                           onClick={() => setIsOpenModal(true)}></i>
                    </div>
                </div>
            </li>
            {isOpenModal &&
                <CommonModal content={'정말로 삭제할까요?'}
                             submitBtnText={'네'}
                             cancelBtnText={'아니요'}
    ...
    return (
        <div className="max-w-md mx-auto">
            ...
            <div className="relative">
                ...
                <input ref={inputRef} value={content} onChange={(event) => {
...
const TodoList = () => {
    ...
    return (
        <React.Fragment>
            ...
            {loading ? <Spinner/> :
                <ul id="todo-list" className="max-w-md divide-y divide-gray-200 dark:divide-gray-700 mx-auto">
                    {items.map((item, idx) => {
                        return (
...
const Ad = () => {
    return (
        <div
            className="fixed top-0 left-0 w-56 max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mx-auto">
            <a href="#">
...
const App = () => {
    ...
    return (
        <React.Fragment>
            ...
            <h1 className="text-3xl text-center pt-5 pb-5 font-bold">TODO List</h1>
            <TodoList/>
            <Ad/>
        </React.Fragment>
    )
